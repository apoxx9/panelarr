...
namespace NzbDrone.Test.Common
{
    public class NzbDroneRunner
    {
        ...
        public void Start(bool enableAuth = false)
        {
            ...
            string panelarrConsoleExe;
            if (OsInfo.IsWindows)
            {
                panelarrConsoleExe = "Panelarr.Console.exe";
            }
            else
            {
                panelarrConsoleExe = "Panelarr";
            }

            _startupLog = new List<string>();
            if (BuildInfo.IsDebug)
            {
                Start(Path.Combine(TestContext.CurrentContext.TestDirectory, "..", "..", "_output", "net6.0", panelarrConsoleExe));
            }
            else
            {
                Start(Path.Combine(TestContext.CurrentContext.TestDirectory, "..", "bin", panelarrConsoleExe));
            }
            ...
            while (true)
            {
                ...
                {
                    TestContext.Progress.WriteLine("Panelarr has exited unexpectedly");
                    Thread.Sleep(2000);
                ...
                {
                    _startupLog = null;
                    TestContext.Progress.WriteLine($"Panelarr {Port} is started. Running Tests");
                    return;
                }

                TestContext.Progress.WriteLine("Waiting for Panelarr to start. Response Status : {0}  [{1}] {2}", statusCall.ResponseStatus, statusCall.StatusDescription, statusCall.ErrorException.Message);
        ...
        public void Kill()
        {
            try
            {
                if (_nzbDroneProcess != null)
                {
                    ...
                    if (_nzbDroneProcess.HasExited)
                    {
                        var log = File.ReadAllLines(Path.Combine(AppData, "logs", "panelarr.trace.txt"));
        ...
        public void KillAll()
        {
            try
            {
                ...
                _processProvider.KillAll(ProcessProvider.PANELARR_CONSOLE_PROCESS_NAME);
                _processProvider.KillAll(ProcessProvider.PANELARR_PROCESS_NAME);
            }
            catch (InvalidOperationException)
            {
                // May happen if the process closes while being closed
        ...
        private void Start(string outputNzbdroneConsoleExe)
        {
            ...
            if (PostgresOptions?.Host != null)
            {
                ...
                envVars.Add("Panelarr__Postgres__Port", PostgresOptions.Port.ToString());
                envVars.Add("Panelarr__Postgres__User", PostgresOptions.User);
                envVars.Add("Panelarr__Postgres__Password", PostgresOptions.Password);
                envVars.Add("Panelarr__Postgres__MainDb", PostgresOptions.MainDb);
                envVars.Add("Panelarr__Postgres__LogDb", PostgresOptions.LogDb);
                envVars.Add("Panelarr__Postgres__CacheDb", PostgresOptions.CacheDb);
