    ...
    public class AuthenticationController : Controller
    {
        ...
        public async Task<IActionResult> Login([FromForm] LoginResource resource, [FromQuery] string returnUrl = null)
        {
            ...
            catch (CryptographicException e)
            {
                ...
                {
                    _logger.Error(e, "Failed to authenticate user due to corrupt XML. Please remove all XML files from {0} and restart Panelarr", Path.Combine(_appFolderInfo.AppDataFolder, "asp"));
                }
                else
                {
