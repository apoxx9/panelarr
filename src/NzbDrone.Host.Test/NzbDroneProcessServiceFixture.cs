    ...
    public class NzbDroneProcessServiceFixture : TestBase<SingleInstancePolicy>
    {
        ...
        public void Setup()
        {
            ...
            Mocker.GetMock<IProcessProvider>()
                  .Setup(s => s.FindProcessByName(ProcessProvider.PANELARR_CONSOLE_PROCESS_NAME))
                  .Returns(new List<ProcessInfo>());

            Mocker.GetMock<IProcessProvider>()
                  .Setup(s => s.FindProcessByName(ProcessProvider.PANELARR_PROCESS_NAME))
                  .Returns(new List<ProcessInfo>());
        }
        ...
        {
            Mocker.GetMock<IProcessProvider>()
                  .Setup(c => c.FindProcessByName(ProcessProvider.PANELARR_CONSOLE_PROCESS_NAME))
                  .Returns(new List<ProcessInfo>
                           {
        ...
        {
            Mocker.GetMock<IProcessProvider>()
                  .Setup(c => c.FindProcessByName(ProcessProvider.PANELARR_PROCESS_NAME))
                  .Returns(new List<ProcessInfo>
                           {
                               new ProcessInfo { Id = CURRENT_PROCESS_ID },
                               new ProcessInfo { Id = 10 }
