    ...
    public abstract class TransmissionBase : TorrentClientBase<TransmissionSettings>
    {
        ...
        public override IEnumerable<DownloadClientItem> GetItems()
        {
            ...
            foreach (var torrent in torrents)
            {
                ...
                if (torrent.Eta >= 0)
                {
                    ...
                    catch (Exception) when (torrent.Eta > TimeSpan.MaxValue.TotalSeconds)
                    {
        ...
        protected ValidationFailure TestConnection()
        {
            ...
            catch (DownloadClientAuthenticationException ex)
            {
                ...
                {
                    DetailedDescription = string.Format("Please verify your username and password. Also verify if the host running Panelarr isn't blocked from accessing {0} by WhiteList limitations in the {0} configuration.", Name)
                };
            }
