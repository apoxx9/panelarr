...
namespace NzbDrone.Core.DecisionEngine.Specifications.RssSync
{
    public class MonitoredBookSpecification : IDecisionEngineSpecification
    {
        ...
        public virtual Decision IsSatisfiedBy(RemoteBook subject, SearchCriteriaBase searchCriteria)
        {
            ...
            var monitoredCount = subject.Books.Count(issue => issue.Monitored);
            ...
            {
                _logger.Debug("Issue is not monitored. Rejecting", monitoredCount, subject.Books.Count);
                return Decision.Reject("Issue is not monitored");
            }

            if (monitoredCount == 0)
            {
                _logger.Debug("No issues in the release are monitored. Rejecting", monitoredCount, subject.Books.Count);
            }
            else
            {
                _logger.Debug("Only {0}/{1} issues in the release are monitored. Rejecting", monitoredCount, subject.Books.Count);
            }
