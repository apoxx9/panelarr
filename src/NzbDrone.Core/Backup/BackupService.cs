...
namespace NzbDrone.Core.Backup
{
    ...
    public class BackupService : IBackupService, IExecute<BackupCommand>
    {
        ...
        public static readonly Regex BackupFileRegex = new Regex(@"panelarr_backup_(v[0-9.]+_)?[._0-9]+\.zip", RegexOptions.Compiled | RegexOptions.IgnoreCase);
                             ...
                             Logger logger)
        {
            ...
            _backupTempFolder = Path.Combine(_appFolderInfo.TempFolder, "panelarr_backup");
        }
        ...
        public void Backup(BackupType backupType)
        {
            ...
            var backupFilename = $"panelarr_backup_v{BuildInfo.Version}_{dateNow:yyyy.MM.dd_HH.mm.ss}.zip";
            ...
            _diskProvider.DeleteFile(Path.Combine(_backupTempFolder, "panelarr.db-journal"));
        ...
        public void Restore(string backupFileName)
        {
            if (backupFileName.EndsWith(".zip"))
            {
                ...
                var temporaryPath = Path.Combine(_appFolderInfo.TempFolder, "panelarr_backup_restore");
                ...
                foreach (var file in _diskProvider.GetFiles(temporaryPath, false))
                {
                    ...
                    if (fileName.Equals("panelarr.db", StringComparison.InvariantCultureIgnoreCase))
                    {
