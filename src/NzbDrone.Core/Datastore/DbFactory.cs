    ...
    public class DbFactory : IDbFactory
    {
        ...
        private void CreateMain(string connectionString, MigrationContext migrationContext, DatabaseType databaseType)
        {
            ...
            catch (SQLiteException e)
            {
                ...
                if (OsInfo.IsOsx)
                {
                    throw new CorruptDatabaseException("Database file: {0} is corrupt, restore from backup if available. See: https://wiki.servarr.com/panelarr/faq#i-use-panelarr-on-a-mac-and-it-suddenly-stopped-working-what-happened", e, fileName);
                }

                throw new CorruptDatabaseException("Database file: {0} is corrupt, restore from backup if available. See: https://wiki.servarr.com/panelarr/faq#i-am-getting-an-error-database-disk-image-is-malformed", e, fileName);
            }
                ...
                if (e.InnerException is SocketException)
                {
                    ...
                    while (true)
                    {
                        ...
                        catch (Exception ex)
                        {
                            ...
                            throw new PanelarrStartupException(ex, "Error creating main database");
                        }
                    }
                }
                else
                {
                    throw new PanelarrStartupException(e, "Error creating main database");
                }
            }
            catch (Exception e)
            {
        ...
        private void CreateLog(string connectionString, MigrationContext migrationContext, DatabaseType databaseType)
        {
            ...
            {
                throw new PanelarrStartupException(e, "Error creating log database");
            }
        }
    }
}
