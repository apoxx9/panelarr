...
namespace NzbDrone.Core.Notifications.Notifiarr
{
    ...
    public class NotifiarrProxy : INotifiarrProxy
    {
        ...
        private void ProcessNotification(WebhookPayload payload, NotifiarrSettings settings)
        {
            try
            {
                var request = new HttpRequestBuilder(URL + "/api/v1/notification/panelarr")
            ...
            catch (HttpException ex)
            {
                ...
                switch ((int)responseCode)
                {
                    ...
                    case 400:
                        ...
                        // between Panelarr and Notifiarr for a specific event, but shouldn't stop all events.
                        _logger.Warn("HTTP 400 - Unable to send notification. Ensure Panelarr Integration is enabled & assigned a channel on Notifiarr");
                        break;
                    case 502:
                    case 503:
                    case 504:
