                ...
                try
                {
                    ...
                    if (files.Any())
                    {
                        ...
                        if (!_diskProvider.FolderExists(libraryFolder))
                        {
                            ...
                            {
                                return new HealthCheck(GetType(), HealthCheckResult.Error, $"Calibre server for root folder {folder.Name} places downloads in {libraryFolder} but Panelarr cannot see this directory.  You may need to adjust the folder's permissions or add a remote path mapping if calibre is running in docker", "#permissions-error");
                            }
                        }
                        ...
                        if (!_diskProvider.FileExists(file))
                        {
                            ...
                            {
                                return new HealthCheck(GetType(), HealthCheckResult.Error, $"Calibre server for root folder {folder.Name} listed file {file} but Panelarr cannot see this file.  Review permissions for {libraryFolder}", "#permissions-error");
                            }
                        }
