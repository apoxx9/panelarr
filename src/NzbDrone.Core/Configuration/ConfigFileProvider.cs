...
namespace NzbDrone.Core.Configuration
{
    ...
    public class ConfigFileProvider : IConfigFileProvider
    {
        ...
        public string PostgresMainDb => _postgresOptions?.MainDb ?? GetValue("PostgresMainDb", "panelarr-main", persist: false);
        public string PostgresLogDb => _postgresOptions?.LogDb ?? GetValue("PostgresLogDb", "panelarr-log", persist: false);
        public string PostgresCacheDb => _postgresOptions?.CacheDb ?? GetValue("PostgresCacheDb", "panelarr-cache", persist: false);
        ...
        private XDocument LoadConfigFile()
        {
            try
            {
                lock (Mutex)
                {
                    if (_diskProvider.FileExists(_configFile))
                    {
                        ...
                        {
                            throw new InvalidConfigFileException($"{_configFile} is empty. Please delete the config file and Panelarr will recreate it.");
                        }

                        if (contents.All(char.IsControl))
                        {
                            throw new InvalidConfigFileException($"{_configFile} is corrupt. Please delete the config file and Panelarr will recreate it.");
                        }
                        ...
                        if (config.Count != 1)
                        {
                            throw new InvalidConfigFileException($"{_configFile} is invalid. Please delete the config file and Panelarr will recreate it.");
                        }
            ...
            {
                throw new InvalidConfigFileException($"{_configFile} is corrupt is invalid. Please delete the config file and Panelarr will recreate it.", ex);
            }
            catch (UnauthorizedAccessException ex)
            {
                throw new AccessDeniedConfigFileException($"Panelarr does not have access to config file: {_configFile}. Please fix permissions", ex);
            }
        }
