...
namespace NzbDrone.Update.UpdateEngine
{
    ...
    public class InstallUpdateService : IInstallUpdateService
    {
        ...
        public void Start(string installationFolder, int processId)
        {
            ...
            _logger.Info("Updating Panelarr from version {0} to version {1}", _detectExistingVersion.GetExistingVersion(installationFolder), BuildInfo.Version);

            Verify(installationFolder, processId);

            if (installationFolder.EndsWith(@"\bin\Panelarr") || installationFolder.EndsWith(@"/bin/Panelarr"))
            {
            ...
            _processProvider.FindProcessByName(ProcessProvider.PANELARR_CONSOLE_PROCESS_NAME);
            _processProvider.FindProcessByName(ProcessProvider.PANELARR_PROCESS_NAME);
            ...
            try
            {
                ...
                if (OsInfo.IsWindows)
                {
                    if (_processProvider.Exists(ProcessProvider.PANELARR_CONSOLE_PROCESS_NAME) || _processProvider.Exists(ProcessProvider.PANELARR_PROCESS_NAME))
                    {
                        _logger.Error("Panelarr was restarted prematurely by external process.");
                        return;
                    }
                }
                ...
                try
                {
                    ...
                    {
                        _diskProvider.SetFilePermissions(Path.Combine(installationFolder, "Panelarr"), "755", null);
                    }
                }
                catch (Exception e)
                {
            ...
            finally
            {
                ...
                else
                {
                    ...
                    for (var i = 0; i < 10; i++)
                    {
                        ...
                        if (_processProvider.Exists(ProcessProvider.PANELARR_PROCESS_NAME))
                        {
                            _logger.Info("Panelarr was restarted by external process.");
                            break;
                        }
                    }
