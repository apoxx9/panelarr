    ...
    public class UpdateServiceFixture : CoreTest<InstallUpdateService>
    {
        ...
        public void Setup()
        {
            if (OsInfo.IsLinux)
            {
                ...
                {
                    FileName = "Panelarr.develop.0.6.2.883.tar.gz",
                    Url = "https://github.com/panelarr/Panelarr/releases/download/v0.6.2.883/Panelarr.develop.0.6.2.883.linux.tar.gz",
            ...
            else
            {
                _updatePackage = new UpdatePackage
                {
                    FileName = "Panelarr.develop.0.6.2.883.zip",
                    Url = "https://github.com/panelarr/Panelarr/releases/download/v0.6.2.883/Panelarr.develop.0.6.2.883.windows.zip",
                    Version = new Version("0.6.2.883")
                };
            }

            Mocker.GetMock<IAppFolderInfo>().SetupGet(c => c.TempFolder).Returns(TempFolder);
            Mocker.GetMock<IAppFolderInfo>().SetupGet(c => c.StartUpFolder).Returns(@"C:\Panelarr".AsOsAgnostic);
            Mocker.GetMock<IAppFolderInfo>().SetupGet(c => c.AppDataFolder).Returns(@"C:\ProgramData\Panelarr".AsOsAgnostic);
            ...
            Mocker.GetMock<IRuntimeInfo>().Setup(c => c.ExecutingApplication).Returns(@"C:\Test\Panelarr.exe");
            ...
            Mocker.GetMock<IDiskProvider>()
                  .Setup(v => v.FileExists(It.Is<string>(s => s.EndsWith("Panelarr.Update".ProcessNameToExe()))))
                  .Returns(true);
        ...
        {
            const string scriptPath = "/tmp/panelarr/update.sh";
        ...
        public void Should_download_and_extract_to_temp_folder()
        {
            ...
            updateSubFolder.GetDirectories("Panelarr").Should().HaveCount(1);
        ...
        {
            Mocker.GetMock<IAppFolderInfo>().SetupGet(c => c.StartUpFolder).Returns(@"C:\Panelarr".AsOsAgnostic);
            Mocker.GetMock<IAppFolderInfo>().SetupGet(c => c.AppDataFolder).Returns(@"C:\Panelarr\AppData".AsOsAgnostic);
