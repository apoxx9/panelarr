...
namespace NzbDrone.Core.Test.MetadataSource.Goodreads
{
    ...
    public class BookInfoProxySearchFixture : CoreTest<BookInfoProxy>
    {
        ...
        [TestCase("Harry Potter and the sorcerer's stone a summary of the novel", null, "Harry Potter and the Sorcerer's Stone (Issue 1): A Summary Of The Novel")]
        ...
        [TestCase("Roald Dahl", 0, typeof(Series), new[] { "Roald Dahl" }, TestName = "author")]
        [TestCase("Roald Dahl", 1, typeof(Issue), new[] { "Going Solo", "Boy: Tales of Childhood" }, TestName = "issue")]
        public void successful_combined_search(string query, int position, Type resultType, string[] expected)
        {
            ...
            if (resultType == typeof(Series))
            {
                var cast = result[position] as Series;
                cast.Should().NotBeNull();
                cast.Name.Should().ContainAny(expected);
            }
            else
            {
                var cast = result[position] as Issue;
                cast.Should().NotBeNull();
