...
namespace NzbDrone.Host
{
    public static class Bootstrap
    {
        ...
        public static void Start(string[] args, Action<IHostBuilder> trayCallback = null)
        {
            try
            {
                Logger.Info("Starting Panelarr - {0} - Version {1}",
                    ...
                    default:
                        {
                            new HostBuilder()
                                ...
                                {
                                    services.Configure<PostgresOptions>(config.GetSection("Panelarr:Postgres"));
                                    services.Configure<AppOptions>(config.GetSection("Panelarr:App"));
                                    services.Configure<AuthOptions>(config.GetSection("Panelarr:Auth"));
                                    services.Configure<ServerOptions>(config.GetSection("Panelarr:Server"));
                                    services.Configure<LogOptions>(config.GetSection("Panelarr:Log"));
                                    services.Configure<UpdateOptions>(config.GetSection("Panelarr:Update"));
                                }).Build();
            ...
            {
                throw new PanelarrStartupException(ex);
            }
        ...
        public static IHostBuilder CreateConsoleHostBuilder(string[] args, StartupContext context)
        {
            ...
            var bindAddress = config.GetValue<string>($"Panelarr:Server:{nameof(ServerOptions.BindAddress)}") ?? config.GetValue(nameof(ConfigFileProvider.BindAddress), "*");
            var port = config.GetValue<int?>($"Panelarr:Server:{nameof(ServerOptions.Port)}") ?? config.GetValue(nameof(ConfigFileProvider.Port), 8787);
            var sslPort = config.GetValue<int?>($"Panelarr:Server:{nameof(ServerOptions.SslPort)}") ?? config.GetValue(nameof(ConfigFileProvider.SslPort), 6868);
            var enableSsl = config.GetValue<bool?>($"Panelarr:Server:{nameof(ServerOptions.EnableSsl)}") ?? config.GetValue(nameof(ConfigFileProvider.EnableSsl), false);
            var sslCertPath = config.GetValue<string>($"Panelarr:Server:{nameof(ServerOptions.SslCertPath)}") ?? config.GetValue<string>(nameof(ConfigFileProvider.SslCertPath));
            var sslCertPassword = config.GetValue<string>($"Panelarr:Server:{nameof(ServerOptions.SslCertPassword)}") ?? config.GetValue<string>(nameof(ConfigFileProvider.SslCertPassword));
        ...
        private static IConfiguration GetConfiguration(StartupContext context)
        {
            ...
            catch (InvalidDataException ex)
            {
                ...
                throw new InvalidConfigFileException($"{configPath} is corrupt or invalid. Please delete the config file and Panelarr will recreate it.", ex);
            }
        }
