...
namespace NzbDrone.Host
{
    ...
    public class SingleInstancePolicy : ISingleInstancePolicy
    {
        ...
        public void PreventStartIfAlreadyRunning()
        {
            if (IsAlreadyRunning())
            {
                _logger.Warn("Another instance of Panelarr is already running.");
        ...
        public void WarnIfAlreadyRunning()
        {
            if (IsAlreadyRunning())
            {
                _logger.Debug("Another instance of Panelarr is already running.");
            }
        }
        ...
        private List<int> GetOtherNzbDroneProcessIds()
        {
            try
            {
                ...
                var otherProcesses = _processProvider.FindProcessByName(ProcessProvider.PANELARR_CONSOLE_PROCESS_NAME)
                                                     .Union(_processProvider.FindProcessByName(ProcessProvider.PANELARR_PROCESS_NAME))
                                                     .Select(c => c.Id)
                ...
                {
                    _logger.Info("{0} instance(s) of Panelarr are running", otherProcesses.Count);
                }
            ...
            {
                _logger.Warn(ex, "Failed to check for multiple instances of Panelarr.");
                return new List<int>();
            }
        }
    }
