...
import { deleteSeriesIssues } from 'Store/Actions/issueActions';
...
const mapDispatchToProps = {
  ...
  dispatchDeleteSeriesIssues: deleteSeriesIssues,
...
Logger.prototype.cleanse = function(message) {
  const apikey = new RegExp(`access_token=${encodeURIComponent(window.Panelarr.apiKey)}`, 'g');
...
class SignalRConnector extends Component {
  ...
  componentDidMount() {
    ...
    const url = `${window.Panelarr.urlBase}/signalr/messages`;

    this.connection = new signalR.HubConnectionBuilder()
      .configureLogging(new Logger(signalR.LogLevel.Information))
      .withUrl(`${url}?access_token=${encodeURIComponent(window.Panelarr.apiKey)}`)
  ...
  handleSeries = (body) => {
    ...
    if (action === 'updated') {
      ...
      this.props.dispatchRemoveItem({ section, id: body.resource.id });
      this.props.dispatchDeleteSeriesIssues({ seriesId: body.resource.id });
    }
  };
...
SignalRConnector.propTypes = {
  ...
  dispatchDeleteSeriesIssues: PropTypes.func.isRequired,
