...
import IssueStudioFilterModalConnector from './IssueshelfFilterModalConnector';
import IssueshelfFooter from './IssueshelfFooter';
import IssueStudioRowConnector from './IssueshelfRowConnector';
import IssueshelfTableHeader from './IssueshelfTableHeader';
import styles from './Issueshelf.css';
...
class Issueshelf extends Component {
  ...
  estimateRowHeight = (width) => {
    ...
    const issuesPerSeries = issueCount / items.length;
    const issueRowsPerSeries = issuesPerSeries / cols;

    // each row is 23px per issue row plus 16px padding
    return issueRowsPerSeries * 23 + 16;
  };
  ...
  render() {
    ...
    return (
      <PageContent title={translate('LibraryMonitor')}>
        <PageToolbar>
          <PageToolbarSection />
        ...
        <div className={styles.pageContentBodyWrapper}>
          ...
          >
            ...
            {
              !isFetching && !!error &&
                <div>{getErrorMessage(error, 'Failed to load series from API')}</div>
            }
